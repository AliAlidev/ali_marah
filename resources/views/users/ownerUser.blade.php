              ...
              @endif
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<img id="demo" style="width:100px" src="/storage/users/{{$owner_user->image}}" ondbclick="myFunction()">
</td>
								<td>{{$owner_user->phone}}</td>
								<td>{{$owner_user->city}}</td>
								<td>{{$owner_user->region}}</td>
								<td>
							
                            
								<form action="/deleteuser/{{$owner_user->id}}"  method="post">
								@csrf
                           {{ method_field('DELETE')}}
